...
passport_entries = PASSPORTS.map do |x|
  x.split(/\s/).reject(&:empty?).map do |y|
    key, value = y.split ':'
    [key, value]
  end.to_h
end
...
valid_passports =
  passport_entries.count {|passport| (%w(byr iyr eyr hgt hcl ecl pid) - passport.keys.uniq).empty? }
